...
namespace Sheyaaka.BLL.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<User?> AuthenticateUserAsync(LoginInputDto loginInputDto)
        {
            ...
            ArgumentNullException.ThrowIfNullOrWhiteSpace(loginInputDto.Email, "Email is required");
            ArgumentNullException.ThrowIfNullOrWhiteSpace(loginInputDto.Password, "Password is required");
        ...
        public async Task<User?> ConfirmEmailAsync(string email, string token)
        {
            ...
            ArgumentNullException.ThrowIfNullOrWhiteSpace(token, "Token is required");
        ...
        public async Task<User?> ResetPasswordAsync(string email, string token, string newPassword)
        {
            ArgumentNullException.ThrowIfNullOrWhiteSpace(email, "Email is required");
            ArgumentNullException.ThrowIfNullOrWhiteSpace(token, "Token is required");
            ArgumentNullException.ThrowIfNullOrWhiteSpace(newPassword, "New password is required");
            var user = await _userRepository.GetByEmailAsync(email);
            if (user == null) return null;
            if (user.Token != token) return null;
        ...
        private async Task ValidateRegistrationInput(RegisterUserInputDto user)
        {
            ...
            ArgumentNullException.ThrowIfNullOrWhiteSpace(user.Email, "Email is required");
            ArgumentNullException.ThrowIfNullOrWhiteSpace(user.Password, "Password is required");
